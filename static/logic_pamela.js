...
var divInfo = []
  Object.keys(response).forEach(function(key) {
    ...
    info.push([response[key].neighborhood,response[key].sale_price,response[key].homes_sold,response[key].count,response[key].num_bikes_available])
    divvy.push([response[key].divvy_lat,response[key].divvy_lng])
    divInfo.push([response[key].name,response[key].capacity])
  });
  console.log(coord)
  console.log(info)
  ...
  for (var i = 0; i < coord.length; i++) {
       ...
       .bindPopup("<h4>" + info[i][0]+ "</h4> <br> <h6> Sale Price: $" + info[i][1] +  "K</h6> <br> <h6> Homes Sold: " + info[i][2] + "</h6> <br> <h6> Total Divvy Stations : " + info[i][3] + "</h6><br> <h6> Divvy Bikes Available : " + info[i][4] + "</h6>")
        .addTo(myMap);
     }

  for (var i = 0; i < divvy.length; i++) {
      L.marker(divvy[i])
      .bindPopup("<h4>" + divInfo[i][0]+ "</h4> <br> <h6> Capacity: " + divInfo[i][1] +  "</h6>")
      .addTo(divvyMarkers);
